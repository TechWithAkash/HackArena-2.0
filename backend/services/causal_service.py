...
MIN_HISTORY = 3   # Lowered: even fresh users get full causal inference
...
def _estimate_with_dowhy(
    ...
    """
    DoWhy causal estimation using the backdoor criterion.
    Uses the dynamically built user-specific causal graph.
    Falls back to linear estimate if DoWhy fails.
    """
    try:
        import dowhy
        ...
        # Fall back to plain linear estimate
        return _estimate_ate_linear(df, treatment, outcome)
...
async def run_causal(
    ...
    async with pool.acquire() as conn:
        # Fetch recent health logs — flat columns match the new schema
        log_rows = await conn.fetch(
            """
            SELECT log_id, heart_rate, steps, sleep, bmi, stress_level, diet_score
            FROM health_logs
            WHERE user_id = $1
        ...
        # Fetch corresponding risk scores
        risk_rows = await conn.fetch(
            """
            SELECT log_id, risk_score
            FROM risk_scores
            WHERE user_id = $1
            ORDER BY timestamp DESC
            LIMIT 90
    ...
    if len(logs) < MIN_HISTORY or len(risk_docs) < MIN_HISTORY:
        return await _causal_from_shap_priors(user_id, log_id, normalized, pool)
    ...
    for treatment in TREATMENTS:
        if treatment not in df.columns:
            continue
        ate = _estimate_with_dowhy(df, treatment, causal_graph=dynamic_graph)
        if ate is not None:
            effects[treatment] = ate   # negative = risk-reducing intervention

    return await _persist_causal(user_id, log_id, normalized, effects, df, pool)
...
async def _causal_from_shap_priors(
    ...
    """
    When insufficient history exists, infer causal effects from SHAP values
    and the domain causal graph (no observational estimation possible yet).
    """
    async with pool.acquire() as conn:
    ...
    return await _persist_causal(user_id, log_id, normalized, effects, None, pool)
...
async def _persist_causal(
    ...
    if not effects:
        ...
        ranked_factors = sorted(
            [{"factor": k, "ate": v, "chain": CAUSAL_CHAINS.get(k, k)}
    ...
    result = {
        ...
        "log_id":         log_id,
        "timestamp":      now,
        "method":         "dowhy_backdoor" if df is not None and len(df) >= MIN_HISTORY else "shap_prior",
