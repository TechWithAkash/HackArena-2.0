...
Handles:
  ...
  - Two-way AI chat: any message to the bot routes to DarpanAI's Groq chat engine
...
async def link_account(token: str, chat_id: int, username: Optional[str]) -> Optional[str]:
    """
    Link a Telegram chat_id to a DarpanAI user via token.
...
async def send_health_alert(
    ...
    rec_text = "\n".join(f"  {i+1}. {_clean(r.get('action', ''))}" for i, r in enumerate(recommendations[:3])) or "  • See DarpanAI for recommendations"
    ...
    msg = f"""{emoji} *DarpanAI Health Alert*
...
━━━━━━━━━━━━━━━
💬 Reply here to chat with Darpan AI
🔗 Open [DarpanAI](http://localhost:3000/simulation) for full analysis"""
...
async def send_anomaly_alert(user_id: str, anomalies: list) -> bool:
    ...
    msg = f"""🚨 *DarpanAI Anomaly Detected*

Your health metrics show abnormal readings:

{lines}

Open DarpanAI to investigate and run a full causal analysis.
💬 Reply here to ask Darpan AI about this."""
...
Your DarpanAI account (`{user_id}`) is now connected to this Telegram.
...
Type /help to see all commands.""")
            ...
            await send_message(chat_id, "❌ *Invalid or expired token.*\n\nGenerate a new one from DarpanAI Settings → Connect Telegram.")
    else:
        await send_message(chat_id, f"""👋 *Welcome to DarpanAI Bot!*

I'm Darpan, your Cognitive Health Twin assistant.

To link your DarpanAI account:
  1. Open DarpanAI → *Settings*
...
async def _handle_status(chat_id: int, user_id: Optional[str]) -> None:
    ...
    if not risk_row:
        await send_message(chat_id, "📊 No health data yet. Submit a check-in from DarpanAI first.")
        return
...
Open DarpanAI for full simulation and recommendations.""")


async def _handle_help(chat_id: int) -> None:
    await send_message(chat_id, """📖 *DarpanAI Bot Commands*

/start `TOKEN` — Link your DarpanAI account
...
async def _handle_chat(chat_id: int, user_id: str, message: str) -> None:
    ...
    try:
        ...
        system_prompt = "You are Darpan, DarpanAI's Cognitive Health Twin assistant. Answer health questions concisely using the patient's data."
...
async def start_bot_polling() -> None:
    ...
    global _offset
    print("[telegram] Bot polling started — @darpanAi_bot")
